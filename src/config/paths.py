...
class DuaIteratePaths:
    ...
    routes
        Routes file (.rou.xml) generated by the last DuaIterate iteration,
        copied out before its scratch folder is deleted. Used for the
        sumo-gui edge-visualization replay (RQ7).
    """
...
DUA_EXTRA = DuaIteratePaths(
    ...
    routes=DUE_DATA_duaIterate / "routes_last_iteration.rou.xml",
)

# Routes file (.rou.xml) of BM's last training episode, copied out of the
# ephemeral SUMO working file (ROUTES) so it survives as an MLflow artifact.
# Used for the sumo-gui edge-visualization replay (RQ7).
ROUTES_LAST_EPISODE_BM = DUE_DATA_BM / "routes_last_episode.rou.xml"
